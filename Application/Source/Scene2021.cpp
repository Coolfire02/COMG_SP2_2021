		...
		if (entry->attacker->getType() == ENTITYTYPE::PLAYER) {
			if (entry->victim->getType() == ENTITYTYPE::CUSTOM) {
				...
				if (entry->victim->getName().find("backDoorHitBox") != std::string::npos)
				{
					for (int i = 0; i < Game::mManager.getCompletableMissions().size(); i++)
					{
						if (Game::mManager.getCompletableMissions().at(i) == MISSIONTYPE::MISSION_SNEAK_INTO_THE_OFFICE || Game::mManager.getCompletableMissions().at(i) == MISSIONTYPE::MISSION_ABDUCT_BIMSTER && interactionTimer > 2) //do && check if next mission has started to disable this 
						{
							if (!player->isDriving())
								Game::uiManager.setUIactive(UI_E_TO_INTERACT);
							if (ePressed && !eHeld && Game::sceneCooldown > 3)
							{
								ISound* door = AudioHandler::getEngine()->play3D(
									...
									LOOPED::NOLOOP);
								eHeld = true;
								Game::mManager.setProgress(MISSIONTYPE::MISSION_SNEAK_INTO_THE_OFFICE, 100.0f); //completed drug collection mission
...
void Scene2021::Render()
{
	...
	for (auto& entity : eManager.getEntities()) {
		if (entity->getType() == ENTITYTYPE::LIVE_NPC) {
			if ((entity->getEntityData()->Translate - player->getEntityData()->Translate).Magnitude() < 100) {
			}
			else {
				continue;
			}		
