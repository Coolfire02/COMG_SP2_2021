...
struct CollidedWith {
	...
	bool cancelled;
	Vector3 plane;
	CollidedWith(Entity* attacker, Entity* victim, bool justCollided, Vector3 plane) : attacker(attacker), victim(victim), justCollided(justCollided), cancelled(false), plane(plane) {};
