...
SceneGuardBuilding::SceneGuardBuilding() :
	eManager(this)
{
	//Scene
	sceneName = "TestScene";
...
void SceneGuardBuilding::Init() {
	...
	Entity* building = new WorldObject(this, GEO_BOSS_BUILDING, "building1");
	building->getEntityData()->SetTransform(0, 0, -100);
	building->getEntityData()->SetScale(1, 1, 1);
	eManager.spawnWorldEntity(building);

	roadblockright = new WorldObject(this, GEO_ROAD_BLOCK, "building1");
	roadblockright->getEntityData()->SetTransform(15, 0, -35);
	roadblockright->getEntityData()->SetRotate(0, 90, 0);
	roadblockright->getEntityData()->SetScale(31, 15, 140);
	eManager.spawnWorldEntity(roadblockright);

	roadblockleft = new WorldObject(this, GEO_ROAD_BLOCK, "building1");
	roadblockleft->getEntityData()->SetTransform(-15, 0, -35);
	roadblockleft->getEntityData()->SetRotate(0, 90, 0);
	roadblockleft->getEntityData()->SetScale(31, 15, 140);
	eManager.spawnWorldEntity(roadblockleft);

	playersCar = new Car(SEDAN, this, "car");
	playersCar->getEntityData()->SetTransform(0, 0, 200);
	playersCar->getEntityData()->SetRotate(0, 180, 0);
	playersCar->getEntityData()->SetScale(2.5, 2.5, 2.5);
	eManager.spawnMovingEntity(playersCar);
	...
	//	Vector3(player->getEntityData()->Translate.x + 6, player->getEntityData()->Translate.y + 6, player->getEntityData()->Translate.z + 6),
	//	Vector3(0, 1, 0));
...
void SceneGuardBuilding::Update(double dt)
{
	...
	if (playersCar->getEntityData()->Translate.z > 0)
	{
		playersCar->getEntityData()->Translate.z = playersCar->getEntityData()->Translate.z - (30 * dt);
	}
	else
	{
		playersCar->getEntityData()->Translate.z = 0;
	}

	if (playersCar->getEntityData()->Translate.x > 22)
	{
		playersCar->getEntityData()->Translate.x = 22;
	}
	...
	if (player->getEntityData()->Translate.x > 22)
	{
		player->getEntityData()->Translate.x = 22;
	}
	...
	if (NPCsKilled == 2)
	{
		if (roadblockright->getEntityData()->Translate.x < 30)
		{
			roadblockright->getEntityData()->Translate.x = roadblockright->getEntityData()->Translate.x + (5 * dt);
		}
		if (roadblockleft->getEntityData()->Translate.x > -30)
		{
			roadblockleft->getEntityData()->Translate.x = roadblockleft->getEntityData()->Translate.x - (5 * dt);
		}
...
void SceneGuardBuilding::InitLights()
{
...
void SceneGuardBuilding::TopDownMapUpdate(double dt)
{
	//top down camera map
...
void SceneGuardBuilding::CollisionHandler(double dt) {
	...
	for (auto& entry : eManager.getEntities()) {
			...
			if (Math::FAbs((entry->getEntityData()->Translate - player->getEntityData()->Translate).Magnitude()) < 6 && !camMap) {
				...
				if (ePressed && !eHeld) {
					...
					if (((Car*)entry)->getPlayer() == nullptr && !player->isDriving()) {
						...
						if (player->getEntityData()->Translate.z <= 0)
						{
		...
		if (entry->getType() == ENTITYTYPE::LIVE_NPC)
		{
			if (Math::FAbs((entry->getEntityData()->Translate - player->getEntityData()->Translate).Magnitude()) < 20) 
			{
				...
				//NPC Chase
				//Change to < 0.8 for teleport, > 1 for moving away

				entry->getEntityData()->SetTransform
				(
					((entry->getEntityData()->Translate.x - player->getEntityData()->Translate.x) * 0.99) + player->getEntityData()->Translate.x,
					0,
					((entry->getEntityData()->Translate.z - player->getEntityData()->Translate.z) * 0.99) + player->getEntityData()->Translate.z
				);
...
void SceneGuardBuilding::Render()
{
...
void SceneGuardBuilding::RenderSkybox() {
	modelStack.PushMatrix();
...
void SceneGuardBuilding::RenderInteraction() {
}
...
//}
//
void SceneGuardBuilding::RenderUI()
{
	Game::RenderUI();
}

//void SceneGuardBuilding::SpawnNPCs(Vector3 v3Tmin, Vector3 v3Tmax, NPCTYPE geoType)
//{
...
void SceneGuardBuilding::SpawnNPCs(Vector3 Position, NPCTYPE geoType)
{
	//int diffX = v3Tmax.x - v3Tmin.x; //get the diff of min and max X
	//int randomX = rand() % diffX + v3Tmin.x; //get random X position from minX to maxX range
